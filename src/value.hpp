	...
	switch (value.type()) {
		...
		case Type::Array: {
			...
			for (const auto& value : arr) {
				if (!first) {
					result.push_back(',');
					if (indentation != noajson::NO_INDENTATION)
						result.push_back(' ');
				}
				first = false;
		...
		case Type::Object: {
			...
			const auto add_line = [&](int depth) {
				if (indentation != noajson::NO_INDENTATION) {
					result.push_back('\n');
					const char iden = indentation == noajson::TAB_INDENTATION ? '\t' : ' ';
					const size_t size = indentation == noajson::TAB_INDENTATION ? depth : depth * indentation;
...
std::size_t std::hash<noajson::Value>::operator()(noajson::Value const& value) const {
	if (value.is_null()) {
		return std::size_t(-1);
	}
	if (value.is_bool()) {
	...
	if (value.is_array()) {
		...
		for (auto const& item : arr) {
			seed = hash_combine(seed, hash<noajson::Value>()(item));
		}
		return seed;
	}
	...
	if (value.is_object()) {
		...
		for (auto& [key, item] : obj) {
			...
			seed = hash_combine(seed, std::hash<noajson::Value>()(item));
		}
		return seed;
	}
