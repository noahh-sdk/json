...
#include <noajson.hpp>
#include <noajson/stl_serialize.hpp>
#include <map>
...
template <>
struct noajson::Serialize<CoolStruct> {
	static noajson::Value to_json(const CoolStruct& cool) {
		return noajson::Object {
			{ "name", cool.name },
			{ "value", cool.value }
		};
	}
	static CoolStruct from_json(const noajson::Value& value) {
		return CoolStruct {
...
TEST_CASE("String serialization") {
    ...
    REQUIRE(obj.dump(noajson::NO_INDENTATION) == "{\"name\":\"wow!\\nmultiline\",\"value\":123}");
    REQUIRE(obj.dump(noajson::TAB_INDENTATION) == "{\n\t\"name\": \"wow!\\nmultiline\",\n\t\"value\": 123\n}");
    REQUIRE(obj.dump(1) == "{\n \"name\": \"wow!\\nmultiline\",\n \"value\": 123\n}");
}
...
TEST_CASE("Parse") {
    auto obj = noajson::parse(COMPLEX_INPUT);
...
TEST_CASE("Dump/parse round trip") {
    ...
    REQUIRE(obj == noajson::parse(obj.dump()));
    REQUIRE(obj == noajson::parse(obj.dump(noajson::NO_INDENTATION)));
    REQUIRE(obj == noajson::parse(obj.dump(noajson::TAB_INDENTATION)));
    REQUIRE(obj == noajson::parse(obj.dump(69)));
}

TEST_CASE("Test custom Object class") {
    auto obj = noajson::parse(R"(
        {
            "key": 5,
            "value": 6,
            "next": 8,
    ...
    using VMap = std::map<std::string, noajson::Value>;
    auto vmap = VMap {
        { "key", 5 },
        { "value", 6 },
        { "next", 8 },
...
TEST_CASE("Test UTF-8 strings") {
    auto obj = noajson::parse("{\"hello\": \"Ol\xC3\xA1!\"}");
